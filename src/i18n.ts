...
const zh = {
  ...
  virtualDisplayUnavailable: "Virtual display 暂未开放",
  virtualDisplayDescription: "已在后端参数模型中预留；待 Panda Mouse Pro 适配完善后再作为实验功能启用。",
  experimental: "实验性",
  shortcuts: "快捷键速查",
...
const en: Record<keyof typeof zh, string> = {
  ...
  virtualDisplayUnavailable: "Virtual display is not available yet",
  virtualDisplayDescription: "Coming later when Panda Mouse Pro adds support.",
...
export function localizeBackendError(locale: Locale, error: unknown) {
  ...
  const exact: Record<string, MessageKey> = {
    ...
    "Virtual display 仍为实验功能，当前版本尚未开放": "virtualDisplayError",
